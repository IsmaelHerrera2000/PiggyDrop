...
// ── GoalCard ─────────────────────────────────────────────────
function GoalCard({ goal, onClick }: { goal: Goal & { category?: Category }; onClick: (g: Goal) => void }) {
  ...
  return (
    ...
    >
      ...
      {!isComplete && catInfo && catInfo.key !== 'todas' && (
        ...
        }}>{catInfo.emoji} {catInfo.label}</div>
      )}
...
// ── TotalPill ────────────────────────────────────────────────
function TotalPill({ totalSaved, currentFilter, onFilterClick }: {
  totalSaved: number; currentFilter: Filter; onFilterClick: (f: Filter) => void
}) {
  const isActive = currentFilter === 'history'
  return (
    <div onClick={() => onFilterClick('history')} style={{
      background: isActive ? 'rgba(255,107,53,0.18)' : 'rgba(255,255,255,0.04)',
      border: `1px solid ${isActive ? 'rgba(255,107,53,0.5)' : 'rgba(255,255,255,0.07)'}`,
      borderRadius: '16px', padding: '16px 14px', textAlign: 'center',
      cursor: 'pointer', transition: 'all 0.2s',
      transform: isActive ? 'translateY(-2px)' : 'none',
      boxShadow: isActive ? '0 0 20px rgba(255,107,53,0.2)' : 'none',
...
// ── StatPill ─────────────────────────────────────────────────
function StatPill({ label, value, color, filterKey, currentFilter, onFilterClick }: {
  label: string; value: number | string; color: string
  filterKey: Filter; currentFilter: Filter; onFilterClick: (f: Filter) => void
}) {
  const isActive = currentFilter === filterKey
  return (
    <div onClick={() => onFilterClick(filterKey)} style={{
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          background: goal.color + '12', border: `1px solid ${goal.color}25`,
          borderRadius: '10px', padding: '10px 14px', marginBottom: '20px',
          fontSize: '12px', color: 'rgba(255,255,255,0.5)', display: 'flex', justifyContent: 'space-between',
        }}>
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <input type="text" inputMode="decimal" value={amount}
            ...
            onBlur={() => { if (amount) setAmountError(validateAmount(amount)) }}
            placeholder="0.00" autoFocus
            style={{
              width: '100%', background: 'rgba(255,255,255,0.05)',
              border: `1px solid ${amountError ? '#ff6060' : amount ? goal.color + '60' : 'rgba(255,255,255,0.1)'}`,
        ...
        <div style={{ marginBottom: '24px' }}>
          <label style={{ fontSize: '11px', fontWeight: '700', color: 'rgba(255,255,255,0.4)', letterSpacing: '1px', display: 'block', marginBottom: '8px' }}>
            NOTA <span style={{ color: 'rgba(255,255,255,0.25)', fontWeight: '400' }}>(opcional)</span>
          </label>
          <input type="text" value={note}
            onChange={(e) => { if (e.target.value.length <= 60) setNote(e.target.value) }}
...
function NewGoalModal({ onClose, onCreate, isPending }: {
  ...
  const handleNumericChange = (val: string, setter: (v: string) => void, field: 'price' | 'initial') => {
    ...
    setter(val)
    if (submitted) {
      const newErrors = validate(name, field === 'price' ? val : price, field === 'initial' ? val : initial)
      setErrors(newErrors)
    }
  }
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '8px' }}>
            {CATEGORIES.filter(c => c.key !== 'todas').map((c) => (
              <button key={c.key} onClick={() => setCategory(c.key)} style={{
                ...
                background: category === c.key ? color + '25' : 'rgba(255,255,255,0.05)',
                border: `1px solid ${category === c.key ? color + '60' : 'rgba(255,255,255,0.1)'}`,
                color: category === c.key ? color : 'rgba(255,255,255,0.5)',
          ...
          <input type="text" value={name}
            onChange={(e) => {
              if (e.target.value.length <= 40) {
                ...
                if (submitted) setErrors((prev) => ({ ...prev, name: validate(e.target.value, price, initial).name }))
              }
            }}
            placeholder="Ej: MacBook Pro M4" autoFocus
            style={{
              width: '100%', background: 'rgba(255,255,255,0.05)',
              border: `1px solid ${errors.name ? '#ff6060' : 'rgba(255,255,255,0.1)'}`,
              borderRadius: '12px', padding: '14px 16px', color: '#f0f0f5', fontSize: '15px',
              outline: 'none', boxSizing: 'border-box' as const, transition: 'border-color 0.2s',
            }}/>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '8px' }}>
          <div style={{ flex: 1 }}>
            ...
            <input type="text" inputMode="decimal" value={price}
              onChange={(e) => handleNumericChange(e.target.value, setPrice, 'price')}
              placeholder="0.00"
              style={{
                width: '100%', background: 'rgba(255,255,255,0.05)',
                border: `1px solid ${errors.price ? '#ff6060' : 'rgba(255,255,255,0.1)'}`,
                borderRadius: '12px', padding: '14px 16px', color: '#f0f0f5', fontSize: '15px',
                outline: 'none', boxSizing: 'border-box' as const, transition: 'border-color 0.2s',
              }}/>
          ...
          <div style={{ flex: 1 }}>
            ...
            <input type="text" inputMode="decimal" value={initial}
              onChange={(e) => handleNumericChange(e.target.value, setInitial, 'initial')}
              placeholder="0.00"
              style={{
                width: '100%', background: 'rgba(255,255,255,0.05)',
                border: `1px solid ${errors.initial ? '#ff6060' : 'rgba(255,255,255,0.1)'}`,
                borderRadius: '12px', padding: '14px 16px', color: '#f0f0f5', fontSize: '15px',
                outline: 'none', boxSizing: 'border-box' as const, transition: 'border-color 0.2s',
              }}/>
  ...
  goals.forEach((g) => {
    if (g.deposits) {
      g.deposits.forEach((d) => {
        allDeposits.push({ ...d, goalName: g.name, goalEmoji: g.emoji, goalColor: g.color, currency: g.currency })
      })
    }
  })
        ...
        Object.entries(grouped).map(([month, deposits]) => (
          <div key={month} style={{ marginBottom: '24px' }}>
            <div style={{
              ...
              letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '10px', paddingLeft: '4px',
            }}>
            ...
            <div style={{
              ...
              {deposits.map((d, i) => (
                <div key={d.id} style={{
                  ...
                  <div style={{ fontFamily: "'Nunito', sans-serif", fontWeight: '800', fontSize: '16px', color: d.goalColor }}>
...
export default function GoalsDashboard({ initialGoals, userId }: {
  ...
  const baseFiltered = (filter === 'active' ? activeGoals : filter === 'completed' ? completedGoals : goals) as (Goal & { category?: Category })[]
    ...
    startTransition(async () => {
      ...
      if (selectedGoal?.id === goalId) {
        setSelectedGoal((prev) => prev ? {
          ...prev,
          saved_amount: prev.saved_amount + amount,
  ...
  const presentCategories = ['todas', ...Array.from(new Set(
    (goals as (Goal & { category?: Category })[]).map(g => g.category ?? 'otro')
  ))] as Category[]
      ...
      <div style={{ maxWidth: '520px', margin: '0 auto', padding: '32px 20px 60px' }}>
        {filter === 'history' ? (
          ...
          <div style={{ animation: 'fadeUp 0.3s ease' }}>
            ...
            <div style={{
              ...
              {selectedGoal.saved_amount < selectedGoal.target_price && getEstimatedDate(selectedGoal) && (
                <div style={{
                  ...
                  background: selectedGoal.color + '18', border: `1px solid ${selectedGoal.color}30`,
                  borderRadius: '20px', padding: '5px 14px', marginBottom: '20px',
                  fontSize: '12px', color: selectedGoal.color, fontWeight: '700',
            ...
            {selectedGoal.deposits && selectedGoal.deposits.length > 0 && (
              <div style={{
                ...
                <div style={{ fontSize: '11px', fontWeight: '700', color: 'rgba(255,255,255,0.4)', letterSpacing: '1px', marginBottom: '16px' }}>
                ...
                {[...selectedGoal.deposits].reverse().map((d, i) => (
                  <div key={d.id} style={{
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                    padding: '12px 0',
                    borderBottom: i < (selectedGoal.deposits?.length ?? 0) - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none',
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <div style={{
                        width: '36px', height: '36px', borderRadius: '10px',
                        background: selectedGoal.color + '20', display: 'flex',
                    ...
                    <div style={{ fontFamily: "'Nunito', sans-serif", fontWeight: '800', fontSize: '16px', color: selectedGoal.color }}>
                      +{selectedGoal.currency}{d.amount.toLocaleString()}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px' }}>
              <div>
                ...
                <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)', marginTop: '4px' }}>
                  {goals.length} metas · {completedGoals.length} completadas
                </div>
              </div>
              <button onClick={() => setShowNewGoal(true)} style={{
                ...
                fontSize: '13px', cursor: 'pointer', boxShadow: '0 8px 20px rgba(255,107,53,0.35)',
              }}>+ Nueva meta</button>
            </div>
            ...
            {/* Category pills — solo aparecen si hay más de 1 categoría */}
                ...
                {presentCategories.map((key) => {
                  ...
                  return (
                    <button key={key} onClick={() => setCategoryFilter(key)} style={{
                      ...
                      background: isActive ? 'rgba(255,107,53,0.2)' : 'rgba(255,255,255,0.05)',
            ...
            {(filter === 'active' || filter === 'completed') && (
              <div style={{
                ...
                <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', fontWeight: '600' }}>
                  Mostrando: <span style={{ color: filter === 'active' ? '#4ECDC4' : '#A78BFA', fontWeight: '700' }}>
                    {filter === 'active' ? 'Metas activas' : 'Completadas'}
                  </span>
                </div>
                <button onClick={() => setFilter('all')} style={{
                  background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)',
                  borderRadius: '8px', padding: '4px 10px', color: 'rgba(255,255,255,0.5)',
            ...
            {filteredGoals.length === 0 && goals.length > 0 && (
              <div style={{ textAlign: 'center', padding: '60px 20px', color: 'rgba(255,255,255,0.25)' }}>
                <div style={{ fontSize: '40px', marginBottom: '12px' }}>
                  {filter === 'completed' ? '🏆' : categoryFilter !== 'todas' ? '🔍' : '⏳'}
                </div>
